...
# ---------- 检索参数 ----------
TOP_K = 5             # 每次检索取最相关的几个切片（W4：3→5，实测 e22 正确片段可进 top-5）

# 拒答阈值：检索最高相似度低于它时，不调用大模型，直接拒答。
# W4 校准说明（按当前索引实测）：库外题中仅 e29(0.307) 被阈值拦截；
# e28(0.315) 实际高于阈值、走分类器判 out_of_kb 拒答；e30(0.416) 被泄漏拦截前置拒答。
# in_kb 最低分 0.438（e13）→ 阈值 0.31 对库内题安全。
SIM_THRESHOLD = 0.31
