...
## v2.0.0 · Agent 二期 B4 展示验收（2026-09-03）★ 作品升级为 Agent

**一句话**：作品二从"RAG 问答"完整升级为"可负责地完成任务的 Agent"——选型测算助手，功能/评测/展示/简历全链路闭环。

**新增**：
- README Agent 章节（任务流程图/设计要点/轨迹评测结果/运行方式）+ 版本演进表至 v2.0.0
- PRD §4a Agent 行为边界从占位升级为已实现（工具白名单/风险分级/停机节点/预算上限）
- 简历条目作品二更新（含 Agent 能力与 B3 评测数字）
- 下一步迭代方向更新（规划器收敛优化/轨迹评分升级/真降级）

**关键结果**：Agent 版轨迹评测——任务成功率 70%、对抗类 100% 安全处理、单任务成本中位 $0.0013；RAG 版三臂评测——省 37.6% 成本、路由准确率 0.963、零瞎编。

**关联**：[README](../README.md) · [PRD](docs/prd.md) · [简历条目](docs/resume-entry.md) · [验收报告](docs/acceptance-b4.md)

## v1.3.0 · Agent 二期 B3 轨迹评测（2026-09-03）

**一句话**：Agent 从"能跑通"到"可评测"——轨迹评测七件套，20 条任务级用例证明"不是刚好跑通一次"。

**新增**：
- `agent_eval.py`：轨迹评测七件套指标（任务成功率六态/必要步骤完成率/执行步骤有效率/工具业务成功率/风险动作拦截率/重试率/预算触顶率+成本轮数分布）
- `data/agent_eval_set.csv`：20 条任务级用例（15 任务 + 1 边界 + 4 对抗，v3 口径）
- `eval.py --mode agent` 入口 + `docs/eval-report-v1.3.0.md` 报告

**关键结果**：任务成功率 70%（14/20）；对抗类（注入/越权/跳过确认）**100% 安全处理**；预算触顶 25%（1 设计用例 + 4 真实 badcase）；零重试；单任务成本中位 $0.0013。
**关键发现**：规划器过度搜索 kb.search 不收敛（4 条用例 8 轮触顶）→ B4 修复候选。

**关联**：[spec-w7-agent](docs/spec-w7-agent.md) · [验收报告](docs/acceptance-b3.md) · [评测报告](docs/eval-report-v1.3.0.md)
